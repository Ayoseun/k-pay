...
import { transferETH } from "./crypto/contract";
...
export function initializeEventListeners(paymentData) {
  ...
  const cryptoSpinner = document.getElementById('crypto-loader');
  ...
  const cryptoPayButtonText = document.getElementById('crypto-pay-button-text');
  ...
  const summaryContainerCrypto = document.getElementById('summary-container-crypto')
  ...
  const cryptoSummaryConfirmBtn = document.getElementById('crypto-summary-button-confirm')
  const cryptoSummaryBackBtn = document.getElementById('crypto-summary-button-go-back')
  ...
  closeBtn.addEventListener('click', (event) => {
    ...
    summaryContainerCrypto.style.display = 'none';
    paymentLoader(document)
  ...
  const paymentHandlers = {
    ...
    3: () => interactWithContract(window,
      cryptoPayButtonText,
      cryptoSpinner,
      cryptoSummaryConfirmBtn,
      cryptoSummaryBackBtn,
      successContainer,
      middleContainer,
      bottomContainer,
      summaryContainerCrypto,
      cryptoDetails
    ),
  ...
  cryptoSummaryConfirmBtn.addEventListener('click', (event) => {
  ...
  cryptoSummaryBackBtn.addEventListener('click', (event) => {
  ...
  connectWalletButton.addEventListener('click', (e) => {
    ...
    summaryContainerCrypto.style.display = 'block';
  })
  //-----------
 ...
 function paymentLoader(document){
  ...
  const connectWallet = (amount, targetChain, exchangeRateSpan, cryptoAmountSpan) => {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        getCryptoPrice(amount, targetChain, exchangeRateSpan, cryptoAmountSpan)
      } catch (error) {
  ...
  async function interactWithContract(window,
    ...
    try {
      ...
      cryptoSummaryConfirmBtn.style.backgroundColor = '#000';
      cryptoSummaryConfirmBtn.disabled = true; // Disable the button
      cryptoSummaryBackBtn.style.display = 'none';
      if (selectedTokenAddress !== null && selectedTokenAddress != undefined && selectedTokenAddress !== "") {
  ...
  function getCryptoPrice(amount, targetChain, exchangeRateSpan, cryptoAmountSpan) {
    fetch(`https://rest.coinapi.io/v1/exchangerate/${targetChain.token}/USD`, {
      method: 'GET',
      headers: {
      ...
      .then(data => {
        ...
        exchangeRateSpan.textContent = `1 ${targetChain.token} = ${USDPrice.toFixed(3)} USD`; // Use toFixed for 3 decimal places
